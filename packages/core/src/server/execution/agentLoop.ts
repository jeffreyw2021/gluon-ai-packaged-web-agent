...
import { executeTurn, type TurnInput } from "./turnExecutor";
...
export async function runAgentLoop(
  ...
  try {
    ...
    for (
      ...
      const result = await executeTurn(turnInput);
    ...
    errorMessage = err instanceof Error ? err.message : "Unknown agent-loop error";
    throw err;
  } finally {
    await finalizeLoop({
      chatJobRunId: input.chatJobRunId,
...
async function finalizeLoop(args: {
  chatJobRunId: string;
  chatId: string;
  userId: string;
  finishReason: string;
  ...
  try {
    ...
    // The event buffer is cleared after the terminal event is published below,
    // so cursor-based replay can deliver run.completed to late-connecting clients.
  } catch (err) {
    console.error("[gluon:loop] clear streaming state failed", err);
  }
  ...
  if (args.errorMessage) {
    try {
      ...
      console.error("[gluon:loop] prune empty assistant messages failed", err);
    }
  }
    ...
    if (nextStatus === "COMPLETED") {
      ...
      if (args.loadedConfig.hooks.onRunEnd) {
        void args.loadedConfig.hooks.onRunEnd({
          ...
          usage: args.usage,
        }).catch((err) => console.error("[gluon:loop] onRunEnd hook failed", err));
      }
    }
  }
  ...
  try {
    ...
    console.error("[gluon:loop] terminal publish failed", err);
  }

  // Clear the event buffer only AFTER publishing the terminal event so
  // cursor-based replay can deliver the final event to reconnecting clients.
  try {
    await clearRunEventBuffer(args.chatJobRunId);
  } catch (err) {
    console.error("[gluon:loop] clear event buffer failed", err);
  }
